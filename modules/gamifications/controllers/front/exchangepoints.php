...
class GamificationsExchangePointsModuleFrontController extends GamificationsFrontController
{
    ...
    public function initContent()
    {
        ...
        foreach ($pointExchangeRewards as &$pointExchangeReward) {
            $pointExchangeReward['type'] = match((int)$pointExchangeReward['reward_type']) {
                GamificationsReward::REWARD_TYPE_POINTS => $this->l('Fixed amount of points', self::FILENAME),
                GamificationsReward::REWARD_TYPE_RANDOM_AMOUNT_OF_POINTS => $this->l('Random amount of points', self::FILENAME),
                GamificationsReward::REWARD_TYPE_DISCOUNT => $this->l('Discount', self::FILENAME),
                GamificationsReward::REWARD_TYPE_FREE_SHIPPING => $this->l('Free shipping', self::FILENAME),
                GamificationsReward::REWARD_TYPE_GIFT => $this->l('Gift', self::FILENAME)
            };
    ...
    public function setMedia()
    {
        ...
        $this->registerStylesheet(
            'module-' . $this->module->name . '-fc-exchangepoints',
            'modules/' . $this->module->name . '/views/css/fc-exchangepoints.css',
            [
                'media' => 'all',
                'priority' => 200,
            ]
