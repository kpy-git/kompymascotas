...
$(document).ready(function () {
    ...
    var $productAttrbiuteHiddenInput = $('#id_product_attribute');

    /**
     * Autocomplete products search in from
     */
    /*
        06/07/2022 - A partir de ahora se quita el buscador de productos y se tiene que usar el sku completo del
                     producto que se va a regalar
     */
    /*
    $productProductInput
    ...
    */
    ...
    document.getElementById('sku').addEventListener('input', (event) => {
        let sku = event.target.value;
        let [id_product, attr=0] = sku.split('-');

        $productHiddenInput.val(id_product);
        $productAttrbiuteHiddenInput.val(attr);
    })
