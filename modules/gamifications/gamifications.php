...
class Gamifications extends Module
{
    ...
    {
        $exchange_url = $this->context->link->getModuleLink($this->name, 'exchangepoints');
        $loyality_url = $this->context->link->getModuleLink($this->name, 'loyality');

        return $this->render('hook/displayCustomerAccountMenu.tpl', [
            'exchange_url' => $exchange_url,
            'exchange_active' => Tools::getCurrentUrl() === $exchange_url,
            'loyality_url' => $loyality_url,
            'loyality_active' => Tools::getCurrentUrl() === $loyality_url,
        ]);
    }
    ...
    public function hookDisplayProductAdditionalInfo(array $params): string
    {
        ...
        $shoppingPointActivity = new GamificationsShoppingPointActivity($this->getEntityManager());
        $possiblePoints        = $shoppingPointActivity->calculateProductPoints($params['product']['id_product'], $params['product']['id_product_attribute']);

        return $this->render('hook/displayProductAdditionalInfo.tpl', [
    ...
    public function hookActionFrontControllerSetMedia(): void
    {
        if ($this->context->controller->getPageName() === 'product') {
            $this->context->controller->registerStylesheet(
                $this->name . 'product-style',
                'modules/' . $this->name . '/views/css/' . $this->name . '-product.css',
                [
                    'media' => 'all',
                    'priority' => 1000,
                ]
            ...
            $this->context->controller->registerJavascript(
                $this->name . 'product-script',
                'modules/' . $this->name . '/views/js/front/' . $this->name . '-product.js',
                [
                    'priority' => 200,
                    'attributes' => 'async',
                ]
