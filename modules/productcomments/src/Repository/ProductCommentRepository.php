...
class ProductCommentRepository extends ServiceEntityRepository
{
    ...
    public function getNumberCommentsByGrade($id_product, $only_validate = true): array
    {
        $sql = 'SELECT grade, count(*) as comments
            FROM ' . $this->databasePrefix . 'product_comment
            where id_product = ' . (int)$id_product . ' and deleted = 0';
        if ($only_validate) {
            $sql .= ' and validate = 1 ';
        }
        $sql .= 'group by grade order by grade desc';
