...
class ProductComments extends Module implements WidgetInterface
{
    ...
    public function hookDisplayHeader(): void
    {
        ...
        if ($this->context->controller instanceof ProductControllerCore) {
            ...
            $jsList[] = '/modules/productcomments/views/js/scrolling.js';
        }
     ...
     * @throws SmartyException
     */
    private function renderProductCommentsList($product): string
    {
        $commentRepository = $this->get('product_comment_repository');
        ...
        $commentsByGrade = array_reduce(
            $commentRepository->getNumberCommentsByGrade($product->id),
        ...
        for ($i = 5; $i >0; $i--) {
            $commentsByGradeNb[] = [
                ...
                'width' => $commentsNb > 0 ? round(($commentsByGrade[$i] ?? 0) * 100 / $commentsNb, 2) : 0
            ];
        }
     ...
     * @throws SmartyException
     */
    public function renderProductCommentModal($product)
    {
        $criterionRepository = $this->get('product_comment_criterion_repository');
