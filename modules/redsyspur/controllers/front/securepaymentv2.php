...
<?php
			...
			$idLog = iniciarLog( Configuration::get( 'REDSYS_LOG' ), $_GET ["idLog"], Configuration::get( 'REDSYS_LOG_SIZE' ) );
			...
			$service = new RESTAuthenticationRequestService ( Configuration::get ( 'REDSYS_CLAVE_TARJETA_INSITE' ), Configuration::get ( 'REDSYS_URLTPV_INSITE' ) );
			...
			$urlDst = $urlKO ? $urlKO : $returnURL_KO;
			...
			if ($result->getResult () == RESTConstants::$RESP_LITERAL_OK) {
				...
				$rds->validateCart ( $cart, $params ["merchant_order"], $_GET['order'], $customer, $params ["amount"], $params ["idCurrency"], $result->getOperation ()->getMerchantIdentifier (), $result->getOperation ()->getCardNumber (), $result->getOperation()->getCardBrand(), $result->getOperation()->getCardType(), $result->getOperation()->getExpiryDate(), $authCode, $respuestaSIS[1], $idLog, $result->getOperation ()->getTransactionType ());

				$returnURL_OK = $Linkobj->getPageLink('order-confirmation') . '?id_cart='.$cart->id.'&id_module='.$rds->id.'&id_order='.Order::getIdByCartId($cart->id).'&key='.$customer->secure_key;
				$urlDst = $urlOK ? $urlOK : $returnURL_OK;
				...
				if(Configuration::get('REDSYS_MANTENER_CARRITO') == 0){
					$rds->validateOrder($params ["idCart"], _PS_OS_CANCELED_, $params ["amount"]/100, $this->module->paymentMethodNameTarjeta, null);
				}
