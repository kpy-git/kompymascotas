...
<div id="redsys-approve-conditions" class="alert alert-warning" style="display: flex; align-items: center; padding: 15px;">
    <strong style="margin-right: 5px;">¡Atención!</strong> Debe aceptar los términos del servicio para continuar con el pago.
</div>

<div id="redsysForm" {if !$approveToShow}style="display: none;"{/if}>
	<div class="col-md-12">
		<div id="insite-form-container" class="form-container">
		</div>
		<div id="insite-form-partes">
	...
	<script>
		var insitePorPartes = {$insitePorPartes};
		var timeOutRedsyspur = setInterval(function () {
			if (typeof getInSiteForm !== typeof undefined) {
				if(insitePorPartes){
					estiloCaja = 'padding: 6px; padding-right: 0px!important;';
					getCardInput('card-number', estiloCaja + 'width: 96%;', 'Número de tarjeta', 'width: 95%');
					getExpirationInput('card-expiration', estiloCaja, 'MM/AA');
					getCVVInput('cvv', estiloCaja, 'CVV');
					getPayButton('boton', '{$btn_style}', '{l s=$btn_text mod='redsys'}', '{$merchant_fuc}', '{$merchant_term}', '{$merchant_order}');
					$("#insite-form-container").hide();
				}else{
					getInSiteForm('insite-form-container','{$btn_style}','{$body_style}','{$form_style}','{$form_text_style}','{l s=$btn_text mod='redsys'}','{$merchant_fuc}','{$merchant_term}','{$merchant_order}', '{$idioma_tpv}', true, true, 'inline', true);
					$("#insite-form-partes").hide();
				}
		...
		function idOperOK() {
			$.ajax({
				...
				data: {
					{if $allow_ref==true}
						"saveRef":document.getElementById("check-guardar").checked,
					{/if}               
		...
		window.addEventListener("message", function receiveMessage(event) {
			if(event.origin.includes("redsys.es")){
				...
				if(document.getElementById("errorCode").value != ""){
					alert(errorCodeLiteral(document.getElementById("errorCode").value));
				}
		...
		function initRedsysInsite(){
				...
				if($("#conditions-to-approve input[type=checkbox]").size()>0 && !$("#conditions-to-approve input[type=checkbox]")[0].checked){
					...
					conditions_to_approve.addEventListener('click', function(){
						if (conditions_to_approve.checked) {
							$("#redsys-approve-conditions").hide();
							...
							$("#redsys-approve-conditions").show();
							$("#redsys-hosted-pay-button").hide();
							$("#redsys-error-tarjeta-nueva").show();
							$("#checkboxGuardar").hide();
						}
			...
			$('form[action=tarjetaInsite]').off('submit').on('submit', function(e){
				payFromModule();
		...
		errorCodeLiterales = {
			...
			"msg8": "El año de caducidad de la tarjeta es incorrecto.",
			...
			"msg17": "La validación por parte del comercio es incorrecta."
		};

		function errorCodeLiteral(errorCode){
			if(errorCode in errorCodeLiterales){
				return errorCodeLiterales[errorCode];
			}else{
				return errorCode;
			}
		}
