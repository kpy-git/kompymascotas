...
function iniciarLog($logLevel, $idLog, $logSize = 0) {

	$GLOBALS["logLevel"] = (int)$logLevel;
	$GLOBALS["logSize"] = (int)$logSize;
	return $idLog;

}

function generateIdLog($logLevel, $logString, $idCart = NULL, $logSize = 0, $force = false) {

	($idCart == NULL) ? srand(make_seed()) : srand(intval($idCart));
	$logSize = empty($logSize) ? 0 : $logSize;
...
function escribirLog($tipo, $idLog, $texto, $logLevel = NULL, $method = NULL) {
	...
	$logSize = $GLOBALS["logSize"];
	$logfilename = dirname(__FILE__).'/../logs/redsysLog.log';
	checkSize($logfilename, $logSize);
...
function checkSize($logfilename, $filesize) {
	$filesize = $filesize*1024*1024;
	if($filesize > 0 && filesize($logfilename) >= $filesize){
		copy($logfilename, $logfilename . "." . date("YmdHis"));
		unlink($logfilename);
	}
}
