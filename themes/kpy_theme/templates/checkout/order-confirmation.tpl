...
{block name='content'}
    {block name='order_confirmation_header'}
        <div class="alert alert-success mt-5" role="alert" tabindex="0">
            {include file='components/page-title-section.tpl' title={l s='Your order is confirmed' d='Shop.Theme.Checkout'}}
        </div>

        <p class="mt-3">
            {l s='Aquí abajo tienes todos los detalles por si quieres revisarlos.' d='Shop.Theme.Checkout'} {l s='An email has been sent to your mail address %email%.' d='Shop.Theme.Checkout' sprintf=['%email%' => $order_customer.email]}
        </p>
        ...
        {* no sé porque no aparece en las traducciones... *}
        {*<p>{l s='Below are all the details in case you want to review them.' d='Shop.Theme.Checkout'}</p>*}
        <p></p>
    {/block}
        ...
        <div class="{$componentName}__details card border-1 {if !$registered_customer_exists}mb-3{else}mb-4{/if}">
            <div class="card-body">
                ...
                <ul class="{$componentName}__details-list">
                    <li>{l s='Número de pedido: %reference%' d='Shop.Theme.Checkout' sprintf=['%reference%' => $order.details.id]}</li>
                    {if $order.details.payment == 'Kpy Cash on delivery'}
                        <li>{l s='Payment method' d='Shop.Theme.Checkout'}: {l s='Pay by Cash on Delivery' d='Modules.Kpycashondelivery.Shop'}</li>
                    {else}
                        <li>{l s='Payment method: %method%' d='Shop.Theme.Checkout' sprintf=['%method%' => $order.details.payment]}</li>
                    {/if}
                    {if !$order.details.is_virtual && !($is_multishipment_enabled|default:false)}
                        <li>{l s='Agencia de transporte: %method%' d='Shop.Theme.Checkout' sprintf=['%method%' => $order.carrier.name]}</li>
                    {/if}
                </ul>
