...
{else}
    <div class="{if !empty($field.errors)} has-error{/if} kpy-form-group {if !empty($field.classes)} {$field.classes}{/if}">

        {if $field.type === 'select'}

            {block name='form_field_item_select'}
                <select class="form-select form-control" name="{$field.name}" {if $field.required}required{/if}>
        ...
        {elseif $field.type === 'countrySelect'}
            ...
            {block name='form_field_item_country'}
                <select
                        class="form-select js-country form-control"
                        name="{$field.name}"
                ...
                <div>
                    {foreach from=$field.availableValues item="label" key="value"}
                        <div class="form-check form-check-inline">
                            ...
                            >
                            <label class="form-check-label kpy-form-check-label" for="field-{$field.name}_{$value}">
                                {$label}
                            </label>
                        </div>
                    {/foreach}
            ...
            {block name='form_field_item_checkbox'}
                <div class="form-check">
                    <input
                            ...
                            {if $field.required}required{/if}
                    >
                    <label class="form-check-label kpy-form-check-label" for="field-{$field.name}">
        ...
        {elseif $field.type === 'birthday'}
            ...
            {block name='form_field_item_birthday'}
                {assign var='day_empty' value={l s='-- day --' d='Shop.Forms.Labels'}}
                {assign var='month_empty' value={l s='-- month --' d='Shop.Forms.Labels'}}
                {assign var='year_empty' value={l s='-- year --' d='Shop.Forms.Labels'}}
                {assign var='start_year' value={'Y'|date}-100}
                {assign var='end_year' value={'Y'|date}}
        ...
        {elseif $field.type === 'textarea'}
            ...
            {block name='form_field_item_textarea'}
                <textarea
                        ...
                        class="form-control"
                        name="{$field.name}"
                        placeholder="{if isset($field.availableValues.placeholder)}{$field.availableValues.placeholder}{else} {/if}"
        ...
        {else}
            ...
            {block name='form_field_item_other'}
                <input
                        ...
                        value="{$field.value}"
                        {if $field.autocomplete}autocomplete="{$field.autocomplete}"{/if}
                        {* siempre meto un espacio en blanco para controlar cuando el input está vacio con css *}
                        placeholder="{if isset($field.availableValues.placeholder)}{$field.availableValues.placeholder}{else} {/if}"
                        {if $field.maxLength}maxlength="{$field.maxLength}"{/if}
        ...
        {if ($field.type !== 'checkbox')}
            ...
            <label class="form-label{if $field.required} required{/if} kpy-form-label{if $field.type === 'radio-buttons'}-radio{/if}" for="field-{$field.name}">
                {$field.label}
            </label>
        {/if}
        ...
        {block name='form_field_comment'}
            {if (!$field.required && !in_array($field.type, ['radio-buttons', 'checkbox']))}
                <div class="form-text optional">{l s='Optional' d='Shop.Forms.Labels'}</div>
            {/if}
        {/block}
    </div>
{/if}
